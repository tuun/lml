...
import { emit, resolveDocument, execScriptSection, compilePageToJs } from './codegen.js';
import { generateApacheHtaccess, generateNginxConf, fileToUrl, } from './server.js';
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        switch (arg) {
            ...
            case '--server-name': {
                const next = args[i + 1];
                if (!next || next.startsWith('-')) {
                    console.error('Error: --server-name requires a value.');
                    process.exit(1);
                }
                serverName = next;
                i++;
            ...
            case '--nginx-root': {
                const next = args[i + 1];
                if (!next || next.startsWith('-')) {
                    console.error('Error: --nginx-root requires a value.');
                    process.exit(1);
                }
                nginxRoot = next;
                i++;
...
async function main() {
    ...
    if (args.mode === 'file') {
        ...
        try {
            ...
            const reader = (path) => {
                try {
                    return parse(readFileSync(join(srcBase, path), 'utf-8'));
                }
                catch {
                    return null;
                }
    ...
    //
    const { srcDir, outDir, title, lang, apache, nginx, custom404, noRouteMap, serverName, nginxRoot, ssl, httpsRedirect, } = args;
