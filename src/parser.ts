import type { Attribute, ASTNode, ElementNode, Document } from './types.js';
 ...
 */
const ATTR_RE =
  /^(?<name>[a-zA-Z_][a-zA-Z0-9_:-]*)(?:=(?:"(?<dq>[^"]*)"|'(?<sq>[^']*)'|(?<uq>\S+)))?$/;
...
function prepareLines(source: string): SourceLine[] {
  return source.split('\n').map((raw, idx) => {
    ...
    for (let i = 0; i < raw.length; i++) {
      if (raw[i] === ' ') indent++;
      else if (raw[i] === '\t') { indent += 2; hasLeadingTab = true; } // treat tab as 2 spaces
      else break;
    }
...
function isBlankOrComment(line: SourceLine): boolean {
  return line.trimmed === '' || COMMENT_RE.test(line.raw);
}

function skip(lines: SourceLine[], i: number): number {
...
function parseAttribute(raw: string, lineNum: number): Attribute {
  const m = ATTR_RE.exec(raw);
  if (!m || !m.groups) {
    throw new ParseError(
      `Invalid attribute syntax: ${JSON.stringify(raw)}`,
      lineNum,
    );
  }
  const { name, dq, sq, uq } = m.groups as Record<string, string | undefined>;
...
function parseElement(
  ...
  if (!openLine.trimmed.startsWith('<')) {
    throw new ParseError(
      `Expected '<tagname', got ${JSON.stringify(openLine.trimmed)}`,
      openLine.lineNum,
    );
  }
  ...
  while (true) {
    ...
    if (i >= lines.length) {
      throw new ParseError(
        `Unexpected end of input — missing '>' to close <${tag}>`,
        openLine.lineNum,
      );
    }
    ...
    if (line.indent === baseIndent) {
      ...
      if (inChildren) {
        throw new ParseError(
          `Expected '>' to close <${tag}>, got ${JSON.stringify(line.trimmed)}`,
          line.lineNum,
        );
      }
      ...
      if (line.hasLeadingTab) {
        console.warn(
          `[lml] Line ${line.lineNum}: attribute indented with a tab character — ` +
      ...
      if (blockM) {
        ...
        while (i < lines.length) {
          const bl = lines[i]!;
          if (bl.trimmed === '}') { i++; closed = true; break; }
          if (bl.trimmed !== '') parts.push(bl.trimmed);
          i++;
        }
        if (!closed) {
          throw new ParseError(
            `Unclosed block attribute '${name}={' — missing '}'`,
            line.lineNum,
          );
        }
      ...
      if (line.indent !== childIndent) {
        throw new ParseError(
          `Inconsistent indentation inside <${tag}> — ` +
            `expected ${childIndent} spaces but got ${line.indent}`,
          line.lineNum,
        );
      }
      ...
      throw new ParseError(
        `Unexpected dedent inside <${tag}> at line ${line.lineNum}`,
        line.lineNum,
      );
    }
  }
...
export function parse(source: string): Document {
  ...
  while (true) {
    ...
    if (!line.trimmed.startsWith('<')) {
      throw new ParseError(
        `Unexpected content at top level: ${JSON.stringify(line.trimmed)}`,
        line.lineNum,
      );
    }
 ...
 */
export function splitScriptAndLml(source: string): { script: string; lml: string } {
  const lines = source.split('\n');
  const sepIdx = lines.findIndex(l => l.trim() === '<lml>');
  if (sepIdx === -1) return { script: '', lml: source };
  return {
